...
BEGIN
    CREATE INDEX IX_Workouts_UserId_Date_Perf
        ON dbo.Workouts (UserId, [Date])
        INCLUDE (DurationMin, WorkoutTypeId);
END;
GO
...
BEGIN
    CREATE INDEX IX_Goals_UserId_Status_Start_End_Perf
        ON dbo.Goals (UserId, [Status], StartDate, EndDate)
        INCLUDE (MetricType, TargetValue);
END;
GO
