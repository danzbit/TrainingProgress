...
BEGIN
    ...
    (
        ...
        WHERE g.UserId = @UserId
          AND g.[Status] = 0
          -- Custom period goals: always include regardless of start/end date vs today.
          -- Rolling/recurring periods: only include if the period window covers today.
          AND (g.PeriodType = 2 OR g.StartDate <= @CurrentDate)
          AND (g.PeriodType = 2 OR g.EndDate IS NULL OR g.EndDate >= @CurrentDate)
