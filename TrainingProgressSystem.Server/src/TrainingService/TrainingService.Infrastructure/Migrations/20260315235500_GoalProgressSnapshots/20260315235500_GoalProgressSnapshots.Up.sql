...
(
    SELECT
        ...
        cv.CurrentValue,
        CASE
            WHEN g.TargetValue <= 0 THEN 0.0
            WHEN (CAST(cv.CurrentValue AS FLOAT) * 100.0 / g.TargetValue) > 100.0 THEN 100.0
            ELSE CAST(cv.CurrentValue AS FLOAT) * 100.0 / g.TargetValue
        END AS Percentage,
        CASE
            WHEN g.[Status] = 1 OR cv.CurrentValue >= g.TargetValue THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
        END AS IsCompleted,
        COALESCE(gp.LastCalculatedAt, SYSUTCDATETIME()) AS LastCalculatedAt
    FROM dbo.Goals g
    LEFT JOIN dbo.GoalProgresses gp ON gp.GoalId = g.Id
    CROSS APPLY
    (
        SELECT COALESCE(gp.CurrentValue, CASE WHEN g.[Status] = 1 THEN g.TargetValue ELSE 0 END) AS CurrentValue
    ) cv
) AS source
ON target.GoalId = source.GoalId
WHEN MATCHED THEN
...
SET g.[Status] = 1
FROM dbo.Goals g
INNER JOIN dbo.GoalProgresses gp ON gp.GoalId = g.Id
WHERE g.[Status] = 0
  AND gp.CurrentValue >= g.TargetValue;
GO
